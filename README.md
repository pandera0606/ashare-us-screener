...
日历按**美股交易日** `YYYY-MM-DD` 归档。美股夏令时收盘对应北京时间**次日 04:00** 之后（界面有提示）。默认打开最近一条有日榜或简报的日期（当前简报为 `2026-08-28`）。已抓取日榜区间：`2026-08-17` 至 `2026-08-28`。
...
```
python tools/fetch_board.py --start 2026-08-17 --end 2026-08-28
```
...
```
python tools/fetch_context.py --start 2026-08-17 --end 2026-08-28
...
定义于 `DailyBriefings.DAYS`（[js/data/briefings.js](js/data/briefings.js)）。人类可读副本在 [briefings/YYYY-MM-DD.md](briefings/2026-08-28.md)，文头写生成时间与保存时间。
...
```
python tools/archive_index.py --us-date 2026-08-28
```
...
- 当前已抓取：`2026-08-17` 至 `2026-08-28`（10 个交易日）。`BYDDY` 无日K，已跳过
